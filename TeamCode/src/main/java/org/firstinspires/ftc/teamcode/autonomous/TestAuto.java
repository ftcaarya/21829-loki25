...
//        telemetry.addData("Robot Heading", "%.2f°", Math.toDegrees(drive.pose.heading.log()));
    ...
    private Action updateDashboard() {
        return p -> {
            ...
            packet.put("Robot X", drive.localizer.getPose().position.x);
            packet.put("Robot Y", drive.localizer.getPose().position.y);
            packet.put("Robot Heading (deg)", Math.toDegrees(drive.localizer.getPose().heading.log()));
